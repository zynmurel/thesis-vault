        ...
        <DialogHeader className="flex flex-row items-center gap-2">
          ...
          <div className="flex flex-col gap-0">
            <DialogTitle className="text-2xl font-bold">
              Scan {qr === "open" ? "Thesis Book" : "Result"}
            </DialogTitle>
            <DialogDescription>
              Scan the thesis QR code to confirm borrowing or returning.
            </DialogDescription>
          </div>
        </DialogHeader>
...
const ScannedView = ({
  ...
  const { data, isLoading } = api.theses.getThesisByQR.useQuery({ thesisId });
            ...
            <div className="px-2">
              {!!isBorrowed.length ? (
                isBorrowed[0]?.status === "PENDING" ? (
                  <Badge className="bg-orange-500 px-3">Requested</Badge>
                ) : (
                  <Badge variant={"outline"}>To Return</Badge>
                )
              ) : (
          ...
          <div className="">
            <div className="flex flex-row items-center justify-between">
              <p className="text-foreground/80 text-sm">
                Student who made the borrow request
              </p>
            </div>
                ...
                <TableBody>
                  <TableRow>
                    <TableCell className="w-32 font-semibold">
                      {isBorrowed[0]?.Student.studentId}
                    </TableCell>
                    <TableCell>
                      <p className="-mt-1 font-semibold">
                        {isBorrowed[0]?.Student.firstName}{" "}
                        {isBorrowed[0]?.Student.middleName}{" "}
                        {isBorrowed[0]?.Student.lastName}
                      </p>
                    </TableCell>
                    <TableCell className="font-bold">
                      {isBorrowed[0]?.Student.courseCode}
                    </TableCell>
                  </TableRow>
                </TableBody>
              </Table>
            ...
            <div className=" mt-2">
              {isBorrowed[0]?.status === "BORROWED" &&
                isBorrowed[0]?.borrowDueAt && (
                  <p className="text-xs">
                    Return Due Date :{" "}
                    <span className={`${isDateBAfterDateA(isBorrowed[0].borrowDueAt, new Date())? "text-red-500" : ""}`}>
