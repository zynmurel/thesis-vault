...
function ShowBorrows() {
  ...
  return (
    <div className="max-h-[100vh] w-full text-xs">
      <CancelBorrowDialog setOpen={setOpen} open={open} />
          ...
          <div className="h-full space-y-3 overflow-auto p-1.5 pb-20">
            ...
            {isLoading ? (
              ...
              data?.map((borrows, index) => {
                ...
                return (
                  <div
                    className={`grid border-b bg-white p-2 ${!index && "border-t"}`}
                    key={borrows.id}
                  >
...
const CancelBorrowDialog = ({
  ...
  const { mutate: declineBorrow, isPending: declineBorrowIsPending } =
    api.mobile.student.declineThesisBorrow.useMutation({
      onSuccess: async () => {
        toast.success("Thesis borrow declined");
        setOpen(null);
        await Promise.all([
          utils.mobile.student.getBorrowsByStatus.invalidate(),
        ]);
      },
      onError() {
  ...
  const handleConfirm = () => {
    ...
    open && declineBorrow({ thesisBorrowId: open });
    console.log("Borrow cancellation confirmed");
  };

  return (
    <Dialog open={!!open} onOpenChange={() => setOpen(null)}>
      <DialogTrigger asChild>
        <Button variant="destructive">Cancel Borrow</Button>
      </DialogTrigger>
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Are you sure you want to cancel your thesis book borrow request?
